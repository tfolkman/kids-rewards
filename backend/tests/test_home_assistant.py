...
class TestHomeAssistantAuth:
    ...
    def test_endpoint_rejects_invalid_key(self, client):
        ...
        response = client.get(
            ...
            headers={"X-HA-API-Key": "wrong-key"}
        )
    ...
    def test_endpoint_accepts_valid_key(self, client, valid_api_key):
        ...
        response = client.get(
            "/api/home-assistant/pet-tasks/today",
            headers={"X-HA-API-Key": valid_api_key}
        )
...
class TestHomeAssistantResponse:
    ...
    def test_filters_todays_tasks_only(self, mock_get_tasks, client, valid_api_key):
        ...
        mock_tasks = [
            models.PetCareTask(
                id="1", schedule_id="s1", pet_id="p1", pet_name="Spike",
                task_name="Feed", assigned_to_kid_id="clara",
                assigned_to_kid_username="Clara", points_value=5,
                due_date=today_start, status=models.PetCareTaskStatus.ASSIGNED,
                created_at=today_start
            ),
            models.PetCareTask(
                id="2", schedule_id="s1", pet_id="p1", pet_name="Spike",
                task_name="Feed", assigned_to_kid_id="clara",
                assigned_to_kid_username="Clara", points_value=5,
                due_date=yesterday, status=models.PetCareTaskStatus.APPROVED,
                created_at=yesterday
            ),
            models.PetCareTask(
                id="3", schedule_id="s1", pet_id="p1", pet_name="Spike",
                task_name="Feed", assigned_to_kid_id="emery",
                assigned_to_kid_username="Emery", points_value=5,
                due_date=tomorrow, status=models.PetCareTaskStatus.ASSIGNED,
                created_at=tomorrow
            ),
        ]
    ...
    def test_task_transformation(self, mock_get_tasks, client, valid_api_key):
        ...
        mock_task = models.PetCareTask(
            id="1", schedule_id="s1", pet_id="p1", pet_name="Spike",
            task_name="Feed Spike", assigned_to_kid_id="clara",
            assigned_to_kid_username="Clara", points_value=5,
            due_date=today_start, status=models.PetCareTaskStatus.ASSIGNED,
            created_at=today_start, description="Morning feeding"
        )
    ...
    def test_status_mapping(self, mock_get_tasks, client, valid_api_key):
        ...
        for db_status, expected_ha_status in status_map:
            mock_task = models.PetCareTask(
                ...
                task_name="Feed", assigned_to_kid_id="clara",
                assigned_to_kid_username="Clara", points_value=5,
                due_date=today, status=db_status, created_at=today
            )
            mock_get_tasks.return_value = [mock_task]
    ...
    def test_overdue_detection(self, mock_get_tasks, client, valid_api_key):
        ...
        mock_task = models.PetCareTask(
            ...
            task_name="Feed", assigned_to_kid_id="clara",
            assigned_to_kid_username="Clara", points_value=5,
            due_date=past_time, status=models.PetCareTaskStatus.ASSIGNED,
    ...
    def test_summary_calculation(self, mock_get_tasks, client, valid_api_key):
        ...
        mock_tasks = [
            models.PetCareTask(
                ...
                task_name="Feed", assigned_to_kid_id="clara",
                assigned_to_kid_username="Clara", points_value=5,
                due_date=today, status=models.PetCareTaskStatus.APPROVED,
                created_at=today
            ),
            models.PetCareTask(
                id="2", schedule_id="s1", pet_id="p1", pet_name="Spike",
                task_name="Clean", assigned_to_kid_id="emery",
                assigned_to_kid_username="Emery", points_value=10,
                due_date=today, status=models.PetCareTaskStatus.ASSIGNED,
                created_at=today
            ),
            models.PetCareTask(
                id="3", schedule_id="s1", pet_id="p1", pet_name="Spike",
                task_name="Exercise", assigned_to_kid_id="aiden",
                assigned_to_kid_username="Aiden", points_value=5,
                due_date=today, status=models.PetCareTaskStatus.PENDING_APPROVAL,
                created_at=today
            ),
        ]
        mock_get_tasks.return_value = mock_tasks
