...
This is a full-stack web application for managing kids' chores and rewards:
- **Frontend**: React SPA with TypeScript, Mantine UI, deployed on AWS Amplify
- **Backend**: FastAPI on AWS Lambda (via Mangum), containerized with Docker
- **Database**: DynamoDB with tables for users, chores, store items, purchases, and requests
- **Auth**: JWT-based authentication with role-based access (Parent/Kid)
...
docker run -d --network kidsrewards-network --name dynamodb-local \
  -p 8001:8000 amazon/dynamodb-local

# Start backend with SAM
cd backend
sam local start-api --docker-network kidsrewards-network \
  --env-vars ../local-env.json --port 3000
```
...
- Parent role required for: creating chores, approving purchases, managing users
- Kid role can: view assigned chores, make purchases, submit requests

### Main Endpoints
- `/auth/*` - Authentication (login, signup, refresh)
- `/chores/*` - Chore management and completion
- `/store/*` - Store items and purchases
- `/users/*` - User management and points
- `/requests/*` - Item/chore requests from kids

### Data Models
- **User**: id, username, role (Parent/Kid), points, parent_id
- **Chore**: id, name, description, points, assigned_to, recurring_type
- **ChoreLog**: Tracks completions with timestamps and approval status
- **StoreItem**: id, name, description, cost, quantity
- **Request**: Kids' requests for new items/chores with approval workflow

## Testing Approach

### Backend
- Unit tests in `backend/tests/`
- Uses pytest with fixtures for database and auth mocking
...
- **Backend**: GitHub Actions → Docker build → ECR → SAM deploy
- **Frontend**: Pushes to main auto-deploy via AWS Amplify
- **Infrastructure**: Defined in `backend/template.yaml` (SAM template)

## Claude Best Practices - Use these best practices at all times when possible and feel free to ask me for more input to make it possible to follow these best practices
