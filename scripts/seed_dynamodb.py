...
def main():
    ...
    parser.add_argument('--purchase-logs-table', type=str, help='Override the purchase logs table name')
    ...
    users_table_name = args.users_table or os.getenv("USERS_TABLE_NAME") or f"{args.environment}-KidsRewardsUsers"
    store_items_table_name = args.store_items_table or os.getenv("STORE_ITEMS_TABLE_NAME") or f"{args.environment}-KidsRewardsStoreItems"
    purchase_logs_table_name = args.purchase_logs_table or os.getenv("PURCHASE_LOGS_TABLE_NAME") or f"{args.environment}-KidsRewardsPurchaseLogs"
        ...
        if kid_user and store_items_data:
            ...
            if len(store_items_data) > 1:
                purchase_logs_to_seed.append({
                    ...
                    "points_spent": Decimal(str(store_items_data[1]["points_cost"])),
            ...
            if len(store_items_data) > 2:
                 purchase_logs_to_seed.append({
                    ...
                    "points_spent": Decimal(str(store_items_data[2]["points_cost"])),
